...
from components.cnn_cam_multihead import EfficientNetB3CAMMultihead


def convert_to_onnx(model: str,
                    input_size: tuple =(1, 3, 224, 224),
                    onnx_file_path: str = 'model.onnx'):
    if model == "vit":
        model_instance = VitRolloutMultihead(visualize=True)
    elif model == "cnn":
        model_instance = EfficientNetB3CAMMultihead()
    else:
