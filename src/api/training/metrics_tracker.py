...
class MetricsTracker:
    ...
    def get_run_metrics(self, run_id: str) -> Union[MetricsResponse, MetricsErrorResponse]:
        ...
        if not csv_file:
            return MetricsErrorResponse(
                error=f"No metrics CSV found for run {run_id}"
            )
    ...
    def get_run_summary(self, run_id: str) -> Union[RunSummary, MetricsErrorResponse]:
        ...
        if metrics.data:
            ...
            for col in metrics.available_columns:
                ...
                values = [row.get(col) for row in metrics.data if row.get(col) is not None]
                if values:
    ...
    def _parse_csv_metrics(self, csv_file: Path, run_id: str) -> Union[MetricsResponse, MetricsErrorResponse]:
            ...
            for _, row in df.iterrows():
                ...
                for col in available_columns:
                    ...
                    if pd.isna(value):
                        ...
                        row_dict[col] = float(value) if isinstance(value, (int, float)) else value
