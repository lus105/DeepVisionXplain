...
class SplitStep(PreprocessingStep):
    def __init__(self, split_ratio: tuple, seed: int = 42, merge_classes: bool = False):
        super().__init__()
    ...
    def _determine_dataset_type(self, data_path: Path) -> DatasetType:
        ...
        if (data_path / self._image_subdir).exists() and (
            data_path / self._label_subdir
        ).exists():
            ...
            if image_folder_count >= 2:
                ...
                raise ValueError(
                    'Unknown dataset structure in image/label subdirectories'
                )
    ...
    def _split_dataset(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'class' not in df.columns:
            raise ValueError(
                "DataFrame must contain a 'class' column for stratified splitting."
            )
        ...
        if train_count < num_classes:
            raise ValueError(
                'Not enough samples to create stratified splits with the current split ratios and number of classes.'
            )
    ...
    def _save_split(
        self, dataframe: pd.DataFrame, output_path: Path, dataset_type: DatasetType
    ) -> None:
